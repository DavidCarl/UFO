...
As we only needed which files had the the city name we decided to add the 'l' option. We then did a time test to see if it made a difference since it had to output less information to our stdout. We then looked into what else could increase our time performance
